...
public class ProgramHelperWindows extends ProgramHelper {
	...
	private final static Logger debugLog = LoggerFactory
			.getLogger(ProgramHelperWindows.class);

	/**
	 * The number of iterations to wait without a window until we assume that
	 * Hearthstone has been minimised
	 */
	...
	/**
	 * Moves Hearthstone window to foreground
	 */
	...
	private HWND getWindowHandle() {
		// Cache the window handle for five seconds to reduce CPU load and
		// (possibly) minimise memory leaks
		long currentTime = System.currentTimeMillis();
		if (currentTime < lastWindowsHandleCheck + 5000) {
			// It has been less than five seconds since the last check, so use
			// the cached value
			return windowHandle;
		} else {
			debugLog.debug("Updating window handle ({}ms since last update)",
					currentTime - lastWindowsHandleCheck);
		...
		User32.INSTANCE.EnumWindows(new WNDENUMPROC() {
			...
			public boolean callback(HWND hWnd, Pointer arg1) {
				...
				Pointer process = Kernel32.OpenProcess(
						...
						pointer.getValue());
				Psapi.GetModuleBaseNameW(process, null, baseNameBuffer,
						STRING_BUFFER_LENGTH);
				String baseNameString = Native.toString(baseNameBuffer);

				// see
				// https://github.com/JeromeDane/HearthStats.net-Uploader/issues/66#issuecomment-33829132
				User32.INSTANCE.GetClassName(hWnd, classNameBuffer,
						STRING_BUFFER_LENGTH);
				String classNameString = Native.toString(classNameBuffer);

				if (baseNameString.equals(processName)
						&& classNameString.equals("UnityWndClass")) {
					windowHandle = hWnd;
					if (windowHandleId == null) {
						windowHandleId = windowHandle.toString();
						if (lastKnownWindowHandleId == null
								|| lastKnownWindowHandleId != windowHandleId) {
							// The window handle has changed, so try to find the
							// location the HearthStats executable. This is used
							// to
							// find the HS log file. Only compatible with
							// Windows Vista and later, so we skip for Windows
							// XP.
							lastKnownWindowHandleId = windowHandleId;
							if (Config.isOsVersionAtLeast(6, 0)) {
								...
								Kernel32.QueryFullProcessImageNameW(process, 0,
										processFileNameBuffer, lpdwSize);
								String processFileNameString = Native
										.toString(processFileNameBuffer);
								if (processFileNameString != null) {
									int lastSlash = processFileNameString
											.lastIndexOf('\\');
									hearthstoneProcessFolder = processFileNameString
											.substring(0, lastSlash);
								}
							}
						}
						_notifyObserversOfChangeTo("Hearthstone window found with process name "
								+ processName);
					}
				}
				return true;
		...
		if (windowHandle == null && windowHandleId != null) {
			_notifyObserversOfChangeTo("Hearthstone window with process name "
					+ processName + " closed");
			windowHandleId = null;
		}
		return windowHandle;
	...
	private BufferedImage _getScreenCaptureWindows(HWND hWnd) {
		...
		if (bounds.toRectangle().width >= 1024) {
			...
			if (_isFullScreen(bounds.toRectangle())) {
				...
				GDI32Extra.INSTANCE.BitBlt(hdcMemDC, 0, 0, width, height,
						hdcWindow, 0, 0, WinGDIExtra.SRCCOPY);
				...
				GDI32.INSTANCE.GetDIBits(hdcWindow, hBitmap, 0, height, buffer,
						bmi, WinGDI.DIB_RGB_COLORS);
		...
		if (!isMinimised) {
			// Hearthstone has brief periods where its window is not displayed,
			// such as during startup and when changing
			// scree size. We don't want to show a warning for these, so we wait
			// a couple of iterations before assuming
	...
	static class Kernel32 {
		...
		public static native int QueryFullProcessImageNameW(Pointer hProcess,
				int dwFlags, char[] lpExeName, int[] lpdwSize);
	}
	...
	public String hearthstoneLogFile() {
		...
		if (hearthstoneProcessFolder != null) {
			logLocation = hearthstoneProcessFolder
