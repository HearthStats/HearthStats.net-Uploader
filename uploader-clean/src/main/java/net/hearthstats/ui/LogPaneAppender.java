...
/**
 * Appender implementation that takes log messages from SLF4J/Logback, formats
 * them and sends them to the LogPane for display.
 */
public class LogPaneAppender extends AppenderBase<ILoggingEvent> {
  ...
  private String formatMessage(ILoggingEvent eventObject) {
    ...
    if (marker == Log.DIVIDER) {
      // Divider is a special case, there's no message to display: it's just a
      // divider
