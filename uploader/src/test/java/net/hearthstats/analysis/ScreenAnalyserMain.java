...
/**
 * This class tests the ScreenAnalyser against a folder of screenshots (which
 * you need to provide yourself). It generates a set of HTML files showing each
 * screenshot with information about what screen it matched for that screenshot.
 * 
 * @author gtch
 */
public class ScreenAnalyserMain {

  private final static Logger log = LoggerFactory.getLogger(ScreenAnalyserMain.class);
  ...
  public static void main(String[] args) throws Exception {
    new ScreenAnalyserMain().htmlScreenMatchTest();
  }
  ...
  public void htmlScreenMatchTest() throws Exception {
    ...
    Assert
        .assertNotNull(
            "No files found in "
                + IMAGE_PATH
                + ". Please make sure you've set the path to a folder that contains screenshots from Hearthstone",
            imageArray);
    ...
    while (page < pageCount) {
      ...
      try (BufferedWriter output = new BufferedWriter(new FileWriter(filename))) {
        ...
        for (int i = (page - 1) * PAGE_SIZE; i < images.size() && i < page * PAGE_SIZE; i++) {
          ...
          output.write("<tr>" + "<td colspan=\"9\" class=\"filename\"><h2>");
          output.write(image.getName());
          output.write("</h2></td>" + "</tr>" + "<tr>" + "<td rowspan=\"7\"><div><img src=\"");
          output.write(image.getName());
          output.write("\" alt=\"");
          output.write(image.getName());
          ...
          try {
            ...
            // We don't need to keep the two match lists separate any more, so
            ...
            if (expectedWidth != bufferedImage.getWidth()
                || expectedHeight != bufferedImage.getHeight()) {
              pixelMap = analyser.calculatePixelPositions(bufferedImage.getWidth(),
  ...
  private void writeScreenSpecificTest(BufferedWriter output, String label, boolean value)
      throws IOException {
    output.write("<li>");
    if (value) {
      output.write("<b>");
  ...
  private void writeScreenSpecificTest(BufferedWriter output, String label, Integer value)
      throws IOException {
    if (value == null) {
  ...
  private void writeScreenSpecificTest(BufferedWriter output, String label, String value)
      throws IOException {
    output.write("<li>");
    if (value == null) {
      output.write(label);
  ...
  private void writePixelValue(BufferedWriter output, BufferedImage image,
      PixelLocation pixelLocation) throws IOException {
  ...
  private void writeHtmlHeader(BufferedWriter output, int page, int pageCount) throws IOException {
    output
        .write("<html>"
            ...
            + ".extra { vertical-align: top; } " + "h2 { margin: 20px 0 0 0; font-size: 14pt; } "
            + ".cd { color: #FFFFFF; text-align: center; font-weight: bold; } "
            + ".cl { color: #000000; text-align: center; font-weight: bold; } "
            + "tr, td, th { vertical-align: middle; padding: 5px 8px; } " + "</style></head>");
    ...
    output.write("<table>" + "<col width=\"400\">" + "<col width=\"250\">" + "<col width=\"250\">"
        + "<col width=\"35\">" + "<col width=\"120\">" + "<col width=\"35\">"
        + "<col width=\"120\">" + "<col width=\"35\">" + "<col width=\"120\">" + "<thead>" + "<tr>"
        + "<th>Image</th>" + "<th>Exact Match<br>Partial Match</th>" + "<th>Extra Tests</th>"
        + "<th colspan=\"6\">Pixels <i>(red, green, blue)</i></th>" + "</tr>" + "</thead><tbody>");
  }
