...
public class BackgroundImageSave {
  ...
  private static final Logger debugLog = LoggerFactory.getLogger(BackgroundImageSave.class);
  ...
  public static void savePngImage(final BufferedImage image, final String filename) {
    ...
    backgroundThreadExecutor.submit(new Runnable() {
      ...
      public void run() {
        try {
          File outputfile = new File((saveFolder == null ? filename : saveFolder + "/" + filename) + ".png");
  ...
  /**
   * Sets the location where images will be saved.
   * @param saveFolder A folder/directory path.
   */
  public static void setSaveFolder(String saveFolder) {
    BackgroundImageSave.saveFolder = saveFolder;
  }
